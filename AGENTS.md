...
- Keep LiveToon custom stage rim and MToon rim separate: preserve the original MToon `_RimColor`/`_RimTexture`/`_RimLightingMix`/`_RimFresnelPower`/`_RimLift` path and expose a distinct intensity so it can be balanced against local-light rim effects.
