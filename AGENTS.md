...
- Keep LiveToon's Forward pass on `LEqual` (`4`) for all render modes; `Equal` can make Opaque/Cutout materials fail depth testing at some camera distances when the depth/GBuffer path and Forward path differ slightly.
...
- Keep LiveToon custom stage rim and MToon rim separate: preserve the original MToon `_RimColor`/`_RimTexture`/`_RimLightingMix`/`_RimFresnelPower`/`_RimLift` path without an extra MToon-only multiplier, and expose `_CustomRimIntensity` only for the local-light custom rim.
